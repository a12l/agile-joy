...
function get_age_data_by_county_and_age_group (county, age_group) {
  let curr_county
  let age_data_by_age_group_and_county = null
  for (const i in age_data) {
    for (const j in age_data[i]) {
      if (j == 'län') { curr_county = age_data[i][j] }
      if (j == age_group && county == curr_county) { age_data_by_age_group_and_county = parseInt(age_data[i][j]) }
    }
  }
